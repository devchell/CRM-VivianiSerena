...
export default function DefinirSenhaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (form.new !== form.confirm) { toast.error('As senhas não coincidem'); return }
    if (form.new.length < 8) { toast.error('Mínimo de 8 caracteres'); return }
    setSaving(true)
    try {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-charcoal-400 dark:text-charcoal-500 text-sm mt-2">
            Você recebeu uma senha temporária por e-mail.<br />
        ...
        <div className="card-dark shadow-xl p-8">
          {done ? (
            <div className="text-center py-4">
              ...
              <p className="font-heading text-lg font-semibold text-charcoal dark:text-charcoal-50">Senha criada!</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {form.new && (
                  <div className="mt-2 space-y-1">
                    <div className="flex gap-1">
                      {[1, 2, 3, 4].map(i => (
                        <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= strength ? strength <= 1 ? 'bg-red-400' : strength === 2 ? 'bg-yellow-400' : strength === 3 ? 'bg-blue-400' : 'bg-green-400' : 'bg-charcoal-200 dark:bg-charcoal-700'}`} />
                      ))}
                    </div>
                    <p className="text-xs text-charcoal-400">{['', 'Fraca', 'Razoável', 'Boa', 'Forte'][strength]}</p>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    className={`w-full pl-9 pr-10 py-2.5 text-sm rounded-lg border bg-white dark:bg-charcoal-700 text-charcoal dark:text-charcoal-100 focus:outline-none focus:ring-2 focus:ring-rose-gold/40 ${form.confirm && form.new !== form.confirm ? 'border-red-400' : 'border-blush-300 dark:border-charcoal-600'}`}
                  />
              ...
              <button type="submit" disabled={saving || form.new !== form.confirm || form.new.length < 8}
